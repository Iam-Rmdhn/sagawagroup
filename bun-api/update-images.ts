...
const sampleImages = {
  ktp: "http://localhost:9999/uploads/sample-ktp.jpg",
  transfer: "http://localhost:9999/uploads/sample-transfer.jpg",
};
    ...
    for (const mitra of mitras) {
      ...
      if (mitra.fotoKTP && !mitra.fotoKTP.startsWith("http")) {
        if (mitra.fotoKTP.startsWith("uploaded:")) {
          ...
          updates.fotoKTP = `http://localhost:9999/uploads/${filename}`;
          ...
          updates.fotoKTP = `http://localhost:9999/uploads/${mitra.fotoKTP}`;
        }
      }
      ...
      if (mitra.buktiTransfer && !mitra.buktiTransfer.startsWith("http")) {
        if (mitra.buktiTransfer.startsWith("uploaded:")) {
          ...
          updates.buktiTransfer = `http://localhost:9999/uploads/${filename}`;
          ...
          // Plain filename to URL
          updates.buktiTransfer = `http://localhost:9999/uploads/${mitra.buktiTransfer}`;
        }
      }
